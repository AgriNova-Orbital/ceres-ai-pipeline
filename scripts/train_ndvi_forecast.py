"""Train a CNN+LSTM regression model on the NDVI forecasting dataset.

This script trains a :class:`CnnLstmRegressor` on the scalar-regression NPZ
dataset produced by ``scripts/build_ndvi_forecast_dataset.py``.  It uses MSE
loss and is the recommended training entry-point when the risk band suffers
from target leakage (``risk ≈ 1 − NDVI``).

Quick start
-----------
::

    # 1. Build the dataset (if not already done)
...
def _parse_args(argv: list[str] | None = None) -> TrainArgs:
    p = argparse.ArgumentParser(
        description=(
            "Train CNN+LSTM NDVI forecasting model (scalar regression).\n\n"
            "This script trains a CnnLstmRegressor on the NDVI forecasting\n"
            "dataset produced by scripts/build_ndvi_forecast_dataset.py.\n"
            "It uses MSE loss to predict next-week mean NDVI from the\n"
            "previous W weeks of feature bands."
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    ds = NdviForecastDataset(index_csv=args.index_csv, root_dir=args.root_dir)

    # Infer in_channels from the first sample.
    x0, y0 = ds[0]
    if x0.ndim != 4:
        raise SystemExit(f"Expected X to be (W,C,H,W), got {tuple(x0.shape)}")
    if y0.ndim != 0:
        raise SystemExit(f"Expected y to be a scalar, got {tuple(y0.shape)}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for xb, yb in dl:
            ...
            # Filter out samples with NaN targets within the batch.
            valid_mask = torch.isfinite(yb)
            ...
            preds = model(xb_valid)
            if preds.shape != yb_valid.shape:
                raise RuntimeError(
                    f"Shape mismatch: preds={tuple(preds.shape)} y={tuple(yb_valid.shape)}"
                )

            per_elem = loss_fn(preds, yb_valid)
            loss = per_elem.mean()
        ...
        if n_batches == 0:
            raise SystemExit(
                "No valid batches this epoch; check dataset quality (all y are NaN?)."
            )
        avg = total_loss / max(n_batches, 1)
        if not math.isfinite(avg):
            raise SystemExit(
                "Encountered non-finite epoch loss; check data quality."
            )
        print(
            f"epoch {epoch}/{args.epochs} mse={avg:.6f} "
            f"skipped_nan_y={skipped_nan_y} skipped_grad={skipped_invalid_grad}"
        )
