...
def run_inventory(
    *,
    ...
    if cadence_days <= 0:
        raise ValueError("--cadence-days must be > 0")
    ...
    if not observed:
        raise ValueError("No parseable date keys found in input directory")

    inv = compute_inventory(observed, cadence_days=cadence_days)
