...
class CnnLstmRegressor(metaclass=_LazyTorchModuleMeta):
    """CNN+LSTM regressor for scalar prediction from temporal sequences.

    Uses the same CNN+LSTM backbone as :class:`CnnLstmRisk` but produces a
    single scalar output from the **last** LSTM hidden state, suitable for
    regression tasks like NDVI forecasting.

    Input:  x (B, T, C, H, W)
    Output: y (B,) predictions
    """
    ...
    def forward(self, x: Any):
        ...
        emb = self.proj(feats).reshape(b, t, self.embed_dim)
        out, _ = self.lstm(emb)
        # Use the last time-step hidden state for scalar regression.
